...
#[tokio::test]
async fn zhipu_driver_generates_jwt_for_key_id_secret_and_maps_tool_calls() {
    async fn handler(
    ...
    let app = Router::new()
        .route("/api/paas/v4/chat/completions", post(handler))
    ...
    let auth = state.auth.lock().unwrap().clone().unwrap();
    ...
    assert_eq!(captured["model"], "glm-4");
