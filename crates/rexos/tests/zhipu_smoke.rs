...
async fn zhipu_glm_native_smoke() {
    let api_key = std::env::var("ZHIPUAI_API_KEY")
        .or_else(|_| std::env::var("LOOPFORGE_ZHIPUAI_API_KEY"))
        .expect("set ZHIPUAI_API_KEY (or LOOPFORGE_ZHIPUAI_API_KEY) to run this test");

    let base_url = std::env::var("LOOPFORGE_GLM_BASE_URL")
        .unwrap_or_else(|_| "https://open.bigmodel.cn/api/paas/v4".to_string());
    let model = std::env::var("LOOPFORGE_GLM_MODEL").unwrap_or_else(|_| "glm-4".to_string());
