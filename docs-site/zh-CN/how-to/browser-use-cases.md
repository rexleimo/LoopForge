...
把每个配方都当成独立入口来用。如果浏览器环境还没准备好，先回到 [浏览器自动化](browser-automation.md)。
...
- 如果 LoopForge 找不到可执行文件，可以设置 `LOOPFORGE_BROWSER_CHROME_PATH`。

可选：使用 Playwright（legacy）后端：设置 `LOOPFORGE_BROWSER_BACKEND=playwright`，并按 [浏览器自动化](browser-automation.md) 的步骤安装 Playwright。
