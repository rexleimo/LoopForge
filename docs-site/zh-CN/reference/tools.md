...
如果你想“对 workspace 做事”，优先用独立工具。
如果你想“在 LoopForge 内部创建/查询持久状态”，优先用 runtime-managed tools。

如果你想看整体结构，再读一遍 [运行时架构](../explanation/runtime-architecture.md)。
...
=== "Bash（macOS/Linux）"
    ```bash
    loopforge agent run --workspace . --prompt "<粘贴 prompt>"
    ```
...
- 默认是 headless。如需显示浏览器窗口，可在 `browser_navigate` 传 `headless=false`（或设置 `LOOPFORGE_BROWSER_HEADLESS=0` 作为默认值）。
- `browser_screenshot` 只允许写入 workspace 相对路径（不允许绝对路径、不允许 `..`、不允许通过 symlink 逃逸）。
- 默认后端是 CDP，需要本机存在 Chromium 系浏览器（Chrome/Chromium/Edge）。如果 LoopForge 找不到可执行文件，设置 `LOOPFORGE_BROWSER_CHROME_PATH`。
- 可选远程 CDP：设置 `LOOPFORGE_BROWSER_CDP_HTTP`（例如 `http://127.0.0.1:9222`）。
- 可选远程 tab 选择：设置 `LOOPFORGE_BROWSER_CDP_TAB_MODE=reuse` 以跳过 `/json/new` 并复用已有 page target（默认：`new`）。
- 对 loopback CDP HTTP（`127.0.0.1` / `localhost`），LoopForge 会绕过代理设置，避免企业代理误配置导致本地自动化失效。
- 可选 legacy 后端（Playwright bridge）：设置 `LOOPFORGE_BROWSER_BACKEND=playwright` 并安装 Python + Playwright：
...
```text
使用 browser_navigate 打开 https://example.com（headless=false）。然后保存截图到 .loopforge/browser/example.png，最后 browser_close 关闭浏览器。
```
...
```text
使用 browser_navigate 打开 https://example.com，然后调用 browser_scroll 向下滚动 800，再截图到 .loopforge/browser/scroll.png，最后关闭浏览器。
```
...
```text
使用 browser_navigate 打开 https://example.com，然后 browser_click 点击 \"More information\"，截图到 .loopforge/browser/click.png，最后关闭浏览器。
```
...
```text
在关键步骤后调用 browser_screenshot，把证据保存到 .loopforge/browser/page.png。
```
...
- `memory_store` / `memory_recall` → 共享 KV（持久化在 `~/.loopforge/loopforge.db`）
...
```json
{ "query": "LoopForge harness 长任务", "max_results": 5 }
```

Prompt：

```text
使用 web_search 搜索 \"LoopForge harness 长任务\"，取 5 条结果，并写 notes/search.md（标题 + URL）。
```
...
```text
对 .loopforge/browser/page.png 调用 image_analyze，并把返回 JSON 写到 notes/image_meta.json。
```
...
```json
{ "prompt": "一个写着 LoopForge 的简洁 SVG 徽章", "path": "assets/rexos_badge.svg" }
```
...
- `channel_send`（写入 outbox；用 `loopforge channel drain` 投递）
...
```text
使用 knowledge_add_entity 添加一个实体 LoopForge，然后 knowledge_query 搜索 LoopForge，并把输出写到 notes/knowledge.json。
```
...
```text
使用 knowledge_add_relation 添加 LoopForge -> meos 的关系，然后 knowledge_query 搜索 LoopForge 并写 notes/knowledge.json。
```
...
```text
使用 knowledge_query 搜索 LoopForge，并把输出写到 notes/knowledge.json。
```
...
- 单次执行：`loopforge channel drain`
- 常驻 worker：`loopforge channel worker`

当前支持的 channel：

- `console`：drain 时打印到 stdout
- `webhook`：POST JSON 到 `LOOPFORGE_WEBHOOK_URL`
...
```text
使用 channel_send 入队一条 console 消息（recipient=stdout），内容是 \"Hello from LoopForge\"。然后提示我运行 `loopforge channel drain` 来投递。
```
...
- `workflow_run` 更适合编排 `fs_write`、`shell`、`web_fetch`、浏览器工具这类独立工具
- 目前**不支持**在 workflow step 里继续嵌套 `task_*`、`agent_*`、`knowledge_*`、`schedule_*`、`cron_*`、`channel_send` 这类 runtime-managed tools
...
```text
使用 hand_activate 激活 researcher hand。然后用返回的 agent_id 调用 agent_send，让它 web_search \"LoopForge\" 并总结 3 条 bullet。
```
...
```text
使用 text_to_speech 生成 .loopforge/audio/tts.wav（内容：Hello from LoopForge）。然后对该文件调用 media_describe，并把结果写到 notes/tts_meta.json。
```
...
- 默认禁用：设置 `LOOPFORGE_DOCKER_EXEC_ENABLED=1`
- 可选镜像：`LOOPFORGE_DOCKER_EXEC_IMAGE`（默认 `alpine:3.20`）
