...
#[tokio::test]
async fn reserved_schedule_tools_create_list_and_delete() {
    async fn handler(
                        ...
                        "tool_calls": [
                            ...
                            },
                            {
                                ...
                                "function": {
                                    ...
                                    "arguments": serde_json::to_string(&json!({ "id": "sched1" })).unwrap()
                                }
                            },
                            {
                                "id": "call_4",
    ...
    let out = agent
        .run_session(
            ...
            "s1",
            None,
            "exercise reserved schedule tools",
    ...
    let schedule_lists: Vec<String> = msgs
        .iter()
        .filter(|m| {
            m.role == rexos::llm::openai_compat::Role::Tool
                && m.name.as_deref() == Some("schedule_list")
        })
    ...
    assert_eq!(
        schedule_lists.len(),
        2,
        "schedule_list outputs: {schedule_lists:?}"
    );

    let v1: serde_json::Value =
        serde_json::from_str(&schedule_lists[0]).expect("schedule_list[0] should be json");
    assert_eq!(v1.as_array().map(|a| a.len()), Some(1), "{v1}");
    assert_eq!(
        v1[0].get("id").and_then(|v| v.as_str()),
        Some("sched1"),
        "{v1}"
    );

    let v2: serde_json::Value =
        serde_json::from_str(&schedule_lists[1]).expect("schedule_list[1] should be json");
