...
async fn nvidia_nim_openai_compat_smoke() {
    let api_key = std::env::var("NVIDIA_API_KEY")
        .or_else(|_| std::env::var("LOOPFORGE_NVIDIA_API_KEY"))
        .expect("set NVIDIA_API_KEY (or LOOPFORGE_NVIDIA_API_KEY) to run this test");

    let base_url = std::env::var("LOOPFORGE_NVIDIA_BASE_URL")
        .unwrap_or_else(|_| "https://integrate.api.nvidia.com/v1".to_string());
    let model = std::env::var("LOOPFORGE_NVIDIA_MODEL")
        .unwrap_or_else(|_| "meta/llama-3.2-3b-instruct".to_string());

    let client =
        rexos::llm::openai_compat::OpenAiCompatibleClient::new(base_url, Some(api_key)).unwrap();
