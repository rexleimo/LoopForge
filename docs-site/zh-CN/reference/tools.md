...
- 默认后端是 CDP，需要本机存在 Chromium 系浏览器（Chrome/Chromium/Edge）。如果 LoopForge 找不到可执行文件，设置 `REXOS_BROWSER_CHROME_PATH`。
...
```text
使用 text_to_speech 生成 .rexos/audio/tts.wav（内容：Hello from LoopForge）。然后对该文件调用 media_describe，并把结果写到 notes/tts_meta.json。
```
