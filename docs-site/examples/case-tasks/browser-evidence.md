...
!!! note "If the browser doesn't appear"
    Browser tools are headless by default. Make sure `LOOPFORGE_BROWSER_HEADLESS=0` (or pass `headless=false` on the first `browser_navigate` call).
